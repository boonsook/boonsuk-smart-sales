            ...
            for _, r in df.iterrows():
                ...
                if sb_id:
                    ...
                    try:
                        ...
                        # ถ้า payment_method คอลัมน์ไม่มี → ลองใหม่โดยไม่มี
                        upd2 = {k: v for k, v in upd.items() if k != "payment_method"}
    ...
    if _use_supabase():
        try:
            ...
            try:
                ...
                # ถ้า insert ไม่ได้ (เช่น คอลัมน์ payment_method ยังไม่มีใน Supabase) → ลองใหม่โดยไม่มี payment_method
                row2 = {k: v for k, v in row.items() if k != "payment_method"}
                try:
        ...
        if st.button("💾 บันทึกใบงาน", use_container_width=True, type="primary"):
            if not sv_name.strip():
                ...
                line_text = "\n".join([
                    ...
                    f"🔧 ประเภทงาน: {sv_type}",
                    f"⚡ อาการ/งาน: {_sym}",
                    f"📅 วันที่: {sv_date.strftime('%Y-%m-%d')}",
