...
def line_notify_flex(q: dict, doc_type: str = "quotation") -> bool:
    """ส่ง LINE Flex Message (Bubble Card) สวยงาม"""
    if not LINE_TOKEN or not LINE_USER_ID:
        return False
    import urllib.request, json as _json

    header_text = "📋 ใบเสนอราคา" if doc_type == "quotation" else "🧾 ใบเสร็จรับเงิน"
    header_color = "#1565C0" if doc_type == "quotation" else "#2E7D32"
    badge_text   = "ใหม่" if doc_type == "quotation" else "ชำระแล้ว"

    flex_msg = {
        "type": "flex",
        "altText": f"{header_text} - {safe_text(q.get('customer_name','-'))}",
        "contents": {
            "type": "bubble",
            "header": {
                "type": "box",
                "layout": "vertical",
                "contents": [
                    {"type":"text","text":f"❄️ {STORE_NAME}","color":"#FFFFFF","size":"sm","weight":"bold"},
                    {"type":"text","text":header_text,"color":"#FFFFFF","size":"xl","weight":"bold"},
                ],
                ...
                "contents": [
                    {
                        ...
                        "contents": [
                            ...
                            {"type":"text","text":safe_text(q.get("customer_name","-")),"size":"sm","weight":"bold","flex":4,"wrap":True}
                        ]
                    },
                    {
                        "type": "box", "layout": "horizontal",
                        "contents": [
                            ...
                            {"type":"text","text":safe_text(q.get("customer_phone","-")),"size":"sm","flex":4}
                        ]
                    },
                    {
                        "type": "box", "layout": "horizontal",
                        "contents": [
                            {"type":"text","text":"❄️ รุ่น","size":"sm","color":"#888888","flex":2},
                            {"type":"text","text":f"{safe_text(q.get('model','-'))} {int(q.get('model_btu',0)):,} BTU","size":"sm","flex":4,"weight":"bold","wrap":True}
                        ]
                    },
                    {"type":"separator","margin":"md"},
                    {
                        "type": "box", "layout": "horizontal",
                        "contents": [
                            {"type":"text","text":"💰 ราคาสุทธิ","size":"md","color":"#1565C0","flex":3,"weight":"bold"},
                            {"type":"text","text":f"฿{fmt_baht(q.get('net_total',0))}","size":"md","color":"#c62828","flex":3,"weight":"bold","align":"end"}
                        ]
                    },
                ],
                "paddingAll": "16px"
                ...
                "contents": [
                    {
                        ...
                        "action":{"type":"uri","label":"📞 โทรติดต่อ","uri":f"tel:{STORE_PHONE}"},
                        "flex":1
                    },
                    {
                        "type":"button","style":"secondary",
                        "action":{"type":"uri","label":"🌐 Facebook","uri":STORE_WEB},
                        "flex":1
                    }
                ],
                "paddingAll": "12px"
...
""")
    ...
    # ── ออกใบเสร็จอัตโนมัติ ─────────────────────────
    st.divider()
    st.subheader("🧾 ออกเอกสารอัตโนมัติ")
    auto1, auto2, auto3 = st.columns(3)

    if auto1.button("🧾 ออกใบเสร็จ + บันทึก + LINE", use_container_width=True, type="primary"):
        if not customer_name.strip():
            st.error("กรุณากรอกชื่อลูกค้าก่อน")
        else:
            receipt_no = f"RC-{datetime.today().strftime('%Y%m%d%H%M%S')}"
            paid_data  = {**quote_data, "paid_amount": net_total, "status": "💰 รับเงินแล้ว", "receipt_no": receipt_no}
            log_customer_job(paid_data)
            try:
                pdf_b = build_pdf_receipt(quote_data, receipt_no, is_tax=False)
                fname = f"ใบเสร็จ_{customer_name}_{today_str.replace('/','')}.pdf"
                st.success(f"✅ ออกใบเสร็จสำเร็จ! เลขที่: **{receipt_no}**")
                st.download_button("⬇️ ดาวน์โหลดใบเสร็จ", data=pdf_b, file_name=fname,
                                   mime="application/pdf", use_container_width=True)
            except Exception as e:
                st.warning(f"PDF: {e}")
            ok = line_notify_flex(quote_data, doc_type="receipt")
            if ok:
                st.success("💚 ส่ง LINE Flex Message สำเร็จ!")
            else:
                st.info("LINE: ไม่ได้ส่ง (ไม่มี token หรือ user ID)")

    if auto2.button("📋 ออกใบเสนอราคา + LINE Flex", use_container_width=True):
        if not customer_name.strip():
            st.error("กรุณากรอกชื่อลูกค้าก่อน")
        else:
            try:
                pdf_b = build_pdf_quotation(quote_data)
                fname = f"ใบเสนอราคา_{customer_name}_{today_str.replace('/','')}.pdf"
                st.download_button("⬇️ ดาวน์โหลดใบเสนอราคา", data=pdf_b, file_name=fname,
                                   mime="application/pdf", use_container_width=True)
            except Exception as e:
                st.warning(f"PDF: {e}")
            ok = line_notify_flex(quote_data, doc_type="quotation")
            st.success("💚 ส่ง LINE Flex Message สำเร็จ!") if ok else st.info("LINE: ไม่ได้ส่ง")

    if auto3.button("📑 ออกใบกำกับภาษี + LINE", use_container_width=True):
        if not customer_name.strip():
            st.error("กรุณากรอกชื่อลูกค้าก่อน")
        else:
            receipt_no = f"TX-{datetime.today().strftime('%Y%m%d%H%M%S')}"
            try:
                pdf_b = build_pdf_receipt(quote_data, receipt_no, is_tax=True)
                fname = f"ใบกำกับภาษี_{customer_name}_{today_str.replace('/','')}.pdf"
                st.success(f"✅ ออกใบกำกับภาษีสำเร็จ! เลขที่: **{receipt_no}**")
                st.download_button("⬇️ ดาวน์โหลดใบกำกับภาษี", data=pdf_b, file_name=fname,
                                   mime="application/pdf", use_container_width=True)
            except Exception as e:
                st.warning(f"PDF: {e}")
            line_notify_flex(quote_data, doc_type="receipt")
...
# ══════════════════════════════════════════════
# PAGE 4: DASHBOARD (Enhanced with Plotly)
# ══════════════════════════════════════════════
elif page == "📊 Dashboard":
    try:
    ...
    if "date" in df_log.columns:
        df_log["_date_parsed"] = pd.to_datetime(df_log["date"], dayfirst=True, errors="coerce")
        min_d = df_log["_date_parsed"].min()
        max_d = df_log["_date_parsed"].max()
        if pd.notna(min_d) and pd.notna(max_d):
            df1c, df2c = st.columns(2)
            date_from = df1c.date_input("ตั้งแต่วันที่", value=min_d.date())
            date_to   = df2c.date_input("ถึงวันที่",     value=max_d.date())
            mask_date = (df_log["_date_parsed"].dt.date >= date_from) & (df_log["_date_parsed"].dt.date <= date_to)
            df_log = df_log[mask_date]
    ...
    pending_amt = int(df_log[df_log["status"] != "💰 รับเงินแล้ว"]["net_total"].sum()) if "status" in df_log.columns else 0
    ...
    m6.markdown(f'<div class="metric-card"><h4>ค้างชำระ (฿)</h4><h2>{fmt_baht(pending_amt)}</h2></div>', unsafe_allow_html=True)
    ...
    with ch1:
        ...
        if "_date_parsed" in df_log.columns and df_log["_date_parsed"].notna().any():
            df_m = df_log.copy()
            df_m["เดือน"] = df_m["_date_parsed"].dt.to_period("M").astype(str)
            monthly = df_m.groupby("เดือน")["net_total"].sum().reset_index()
            monthly.columns = ["เดือน","ยอดรวม"]
            monthly = monthly.sort_values("เดือน")
            fig_line = px.bar(monthly, x="เดือน", y="ยอดรวม",
                              color_discrete_sequence=["#1565c0"],
                              labels={"ยอดรวม":"ยอด (฿)"},
                              text_auto=True)
            fig_line.update_layout(margin=dict(t=20,b=0), height=320,
    ...
    with ch2:
        ...
        if "status" in df_log.columns:
            ...
            fig_pie = px.pie(sd, values="จำนวน", names="สถานะ",
                             color_discrete_sequence=px.colors.qualitative.Set3,
                             hole=0.4)
            fig_pie.update_layout(margin=dict(t=20,b=0), height=320,
    ...
    # ── Row 2: Top brands bar + Top models ─────────
    ch3, ch4 = st.columns(2)
    with ch3:
        st.subheader("🏆 ยอดขายแยกแบรนด์/ซีรีส์ (Top 10)")
        if "section" in df_log.columns:
            bd = df_log.groupby("section")["net_total"].sum().reset_index()
            bd.columns = ["ซีรีส์","ยอดรวม"]
            bd = bd.sort_values("ยอดรวม", ascending=True).tail(10)
            fig_bar = px.bar(bd, x="ยอดรวม", y="ซีรีส์", orientation="h",
                             color="ยอดรวม", color_continuous_scale="Blues",
                             text_auto=True)
            fig_bar.update_layout(margin=dict(t=10,b=0), height=360,
                                  coloraxis_showscale=False,
                                  plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)")
            st.plotly_chart(fig_bar, use_container_width=True)

    with ch4:
        st.subheader("📦 รุ่นขายดี (Top 10)")
        if "model" in df_log.columns:
            md = df_log.groupby("model").agg(
                จำนวน=("model","count"),
                ยอดรวม=("net_total","sum")
            ).reset_index().sort_values("จำนวน", ascending=False).head(10)
            fig_md = px.bar(md, x="จำนวน", y="model", orientation="h",
                            color="ยอดรวม", color_continuous_scale="Greens",
                            text_auto=True, labels={"model":"รุ่น"})
            fig_md.update_layout(margin=dict(t=10,b=0), height=360,
                                 coloraxis_showscale=False,
                                 plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)")
    ...
    # ── Row 3: BTU distribution + Staff performance ─
    ch5, ch6 = st.columns(2)
    with ch5:
        st.subheader("📐 การกระจาย BTU ที่ขาย")
        if "model_btu" in df_log.columns:
            btu_data = pd.to_numeric(df_log["model_btu"], errors="coerce").dropna()
            if not btu_data.empty:
                fig_hist = px.histogram(btu_data, nbins=12, color_discrete_sequence=["#0288d1"],
                                        labels={"value":"BTU","count":"จำนวน"})
                fig_hist.update_layout(margin=dict(t=10,b=0), height=280,
                                       plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
                                       showlegend=False)
                st.plotly_chart(fig_hist, use_container_width=True)

    with ch6:
        st.subheader("👤 ยอดขายแยกพนักงาน")
        if "saved_by" in df_log.columns:
            staff_d = df_log.groupby("saved_by").agg(
                งาน=("saved_by","count"), ยอด=("net_total","sum")
            ).reset_index().sort_values("ยอด", ascending=False)
            fig_staff = px.bar(staff_d, x="saved_by", y="ยอด",
                               text_auto=True, color="งาน",
                               color_continuous_scale="Purples",
                               labels={"saved_by":"พนักงาน","ยอด":"ยอดรวม (฿)"})
            fig_staff.update_layout(margin=dict(t=10,b=0), height=280,
                                    coloraxis_showscale=False,
                                    plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)")
...
# ══════════════════════════════════════════════
# PAGE: AI แนะนำสินค้า (Claude AI)
# ══════════════════════════════════════════════
if page == "🤖 AI แนะนำสินค้า":
    st.title("🤖 AI แนะนำสินค้าแอร์")
    st.caption("ใช้ Claude AI วิเคราะห์ความต้องการลูกค้า แนะนำรุ่นที่เหมาะสมที่สุด พร้อมเหตุผล")

    ANTHROPIC_API_KEY = os.environ.get("ANTHROPIC_API_KEY", "")

    ai_tab1, ai_tab2 = st.tabs(["💬 ถามด้วยข้อความ", "📐 ประเมินจากขนาดห้อง"])

    # ── Tab 1: Free-text AI chat ──────────────────
    with ai_tab1:
        st.subheader("💬 บอกความต้องการลูกค้า")
        st.info("อธิบายความต้องการลูกค้า แล้ว AI จะแนะนำรุ่นแอร์ที่เหมาะสมจากสต๊อกของร้าน")

        ai_input = st.text_area(
            "อธิบายความต้องการ",
            placeholder="เช่น: ห้องนอน 3x4 เมตร งบ 15,000 บาท ชอบประหยัดไฟ อยู่คนเดียว ไม่ค่อยโดนแดด",
            height=100
        )
        ai_budget = st.number_input("งบประมาณสูงสุด (บาท, 0 = ไม่จำกัด)", min_value=0, step=1000, value=0)

        if st.button("🤖 ให้ AI แนะนำ", use_container_width=True, type="primary"):
            if not ai_input.strip():
                st.error("กรุณาอธิบายความต้องการก่อนครับ")
            elif not ANTHROPIC_API_KEY:
                st.error("⚠️ ไม่พบ ANTHROPIC_API_KEY — กรุณาตั้งค่า environment variable `ANTHROPIC_API_KEY`")
            else:
                with st.spinner("AI กำลังวิเคราะห์..."):
                    try:
                        import urllib.request, json as _json
                        # Build product catalog for AI
                        df_stk_ai = load_stock()
                        catalog_lines = []
                        for _, r in df_stk_ai.iterrows():
                            budget_ok = (ai_budget == 0) or (int(r.get("price_install",0)) <= ai_budget)
                            if budget_ok:
                                catalog_lines.append(
                                    f"- {r['section']} | Model: {r['model']} | BTU: {int(r.get('btu',0)):,} | "
                                    f"ราคาพร้อมติดตั้ง: {fmt_baht(r.get('price_install',0))} บาท | "
                                    f"สต๊อก: {int(r.get('stock_qty',0))} เครื่อง | "
                                    f"ประกันคอมฯ: {r.get('w_comp','-')}"
                                )
                        catalog_text = "\n".join(catalog_lines[:60]) if catalog_lines else "ไม่มีสินค้าในงบประมาณ"
                        budget_text = f"งบประมาณสูงสุด: {fmt_baht(ai_budget)} บาท" if ai_budget > 0 else "ไม่จำกัดงบประมาณ"

                        system_prompt = f"""คุณคือที่ปรึกษาขายแอร์มืออาชีพของ {STORE_NAME}
ข้อมูลสินค้าในสต๊อกปัจจุบัน (แสดงเฉพาะที่ตรงงบประมาณ):
{catalog_text}

{budget_text}

กรุณาแนะนำแอร์ที่เหมาะสมที่สุด 2-3 รุ่น จากสต๊อกข้างต้นเท่านั้น
ตอบเป็นภาษาไทย กระชับ ชัดเจน โดยมีโครงสร้าง:
1. อันดับ 1 (แนะนำมากสุด): ชื่อรุ่น + ราคา + เหตุผล
2. อันดับ 2: ชื่อรุ่น + ราคา + เหตุผล  
3. ทางเลือกอื่น (ถ้ามี)
4. คำแนะนำเพิ่มเติมสั้นๆ"""

                        payload = _json.dumps({
                            "model": "claude-sonnet-4-20250514",
                            "max_tokens": 1000,
                            "system": system_prompt,
                            "messages": [{"role": "user", "content": ai_input.strip()}]
                        }).encode("utf-8")
                        req = urllib.request.Request(
                        ...
                        st.markdown(f"""<div style="background:#e3f2fd;border-radius:12px;padding:20px;
                            border-left:5px solid #1565c0;line-height:1.8;font-size:15px;">
                            {ai_response.replace(chr(10),'<br>')}
                        </div>""", unsafe_allow_html=True)

                        # Show matching products
                        st.divider()
                        st.subheader("📦 สินค้าที่อาจตรงความต้องการ")
                        df_match = df_stk_ai.copy()
                        if ai_budget > 0:
                            df_match = df_match[pd.to_numeric(df_match["price_install"],errors="coerce") <= ai_budget]
                        if not df_match.empty:
                            disp_cols = ["section","model","btu","price_install","stock_qty","w_comp"]
                            disp_cols = [c for c in disp_cols if c in df_match.columns]
                            st.dataframe(df_match[disp_cols].head(15), use_container_width=True, hide_index=True,
                                         column_config={
                                             "section": "ซีรีส์","model":"รุ่น",
                                             "btu": st.column_config.NumberColumn("BTU",format="%d"),
                                             "price_install": st.column_config.NumberColumn("ราคา (฿)",format="%d"),
                                             "stock_qty": st.column_config.NumberColumn("สต๊อก",format="%d"),
                                             "w_comp":"ประกันคอมฯ"
                                         })
                    except Exception as e:
                        st.error(f"AI error: {e}")

    # ── Tab 2: Room-based recommendation ─────────
    with ai_tab2:
        st.subheader("📐 แนะนำโดยอัตโนมัติจากขนาดห้อง + งบประมาณ")
        st.info("กรอกขนาดห้องและงบประมาณ ระบบจะแนะนำรุ่นที่เหมาะสมจากสต๊อกทันที (ไม่ต้องใช้ API)")

        ai2_c1, ai2_c2 = st.columns(2)
        ai2_room = ai2_c1.selectbox("ประเภทห้อง", list(BTU_FACTORS.keys()), key="ai2_room")
        ai2_sun  = ai2_c2.selectbox("ทิศ/แดด", ["ไม่โดนแดด","โดนแดด (ทิศตะวันตก/ใต้/กระจก)"], key="ai2_sun")
        ai2_r1, ai2_r2, ai2_r3 = st.columns(3)
        ai2_w = ai2_r1.number_input("กว้าง (ม.)", min_value=0.0, step=0.1, key="ai2_w")
        ai2_l = ai2_r2.number_input("ยาว (ม.)",   min_value=0.0, step=0.1, key="ai2_l")
        ai2_h = ai2_r3.number_input("สูง (ม.)",   min_value=0.0, step=0.1, value=2.6, key="ai2_h")
        ai2_people  = st.number_input("จำนวนคน", min_value=1, step=1, value=1, key="ai2_people")
        ai2_budget  = st.number_input("งบประมาณสูงสุด (฿, 0=ไม่จำกัด)", min_value=0, step=1000, value=0, key="ai2_budget")
        ai2_prefer  = st.multiselect("แบรนด์ที่ชอบ (เว้นว่างถ้าไม่มีความชอบ)",
                                      options=sorted(set(p["section"].split()[0] for p in PRODUCTS)),
                                      key="ai2_pref")

        if st.button("🔍 แนะนำรุ่นที่เหมาะสม", use_container_width=True, type="primary", key="ai2_btn"):
            if ai2_w <= 0 or ai2_l <= 0:
                st.error("กรุณากรอกขนาดห้อง")
            else:
                sun_val = "โดนแดด" if "โดนแดด" in ai2_sun else "ไม่โดนแดด"
                btu_need = calculate_btu(ai2_w, ai2_l, ai2_h, sun_val, int(ai2_people), ai2_room)
                cap_need = suggest_capacity(btu_need)

                st.info(f"🌡️ BTU ที่ต้องการ: **{btu_need:,}** | แนะนำขนาด: **{cap_need:,} BTU**")

                df_ai2 = load_stock().copy()
                df_ai2["price_install"] = pd.to_numeric(df_ai2["price_install"], errors="coerce")
                df_ai2["btu"]           = pd.to_numeric(df_ai2["btu"], errors="coerce")
                df_ai2 = df_ai2[(df_ai2["btu"] >= cap_need * 0.8) & (df_ai2["btu"] <= cap_need * 1.3)]
                if ai2_budget > 0:
                    df_ai2 = df_ai2[df_ai2["price_install"] <= ai2_budget]
                if ai2_prefer:
                    mask_brand = df_ai2["section"].apply(lambda s: any(b in str(s) for b in ai2_prefer))
                    df_ai2 = df_ai2[mask_brand]

                df_ai2["_score"] = (df_ai2["btu"] - cap_need).abs() / cap_need
                df_ai2 = df_ai2.sort_values(["_score","price_install"]).head(6)

                if df_ai2.empty:
                    st.warning("ไม่พบสินค้าที่ตรงเงื่อนไข ลองปรับงบประมาณหรือแบรนด์ดูครับ")
                else:
                    st.subheader(f"✅ แนะนำ {len(df_ai2)} รุ่นที่เหมาะสม")
                    for rank, (_, r) in enumerate(df_ai2.iterrows(), 1):
                        badge = "🥇" if rank == 1 else "🥈" if rank == 2 else "🥉" if rank == 3 else f"#{rank}"
                        stock_ok = int(r.get("stock_qty",0)) > 0
                        stock_badge = f'<span style="color:#2e7d32;font-weight:700">✅ มีสต๊อก {int(r.get("stock_qty",0))} เครื่อง</span>' if stock_ok else '<span style="color:#c62828">❌ สต๊อกหมด</span>'
                        st.markdown(f"""
                        <div style="background:{'#e8f5e9' if rank==1 else '#fff'};border-radius:10px;padding:14px 18px;
                            margin-bottom:10px;border-left:5px solid {'#2e7d32' if rank==1 else '#1565c0'};
                            box-shadow:0 1px 4px #0001;">
                          <div style="font-size:18px;font-weight:900">{badge} {safe_text(r.get('section',''))} — <span style="color:#1565c0">{safe_text(r.get('model',''))}</span></div>
                          <div style="margin:6px 0;font-size:14px;color:#555">
                            ❄️ BTU: <b>{int(r.get('btu',0)):,}</b> &nbsp;|&nbsp;
                            💰 ราคา: <b>฿{fmt_baht(r.get('price_install',0))}</b> &nbsp;|&nbsp;
                            🛡️ ประกันคอมฯ: <b>{r.get('w_comp','-')}</b> &nbsp;|&nbsp;
                            {stock_badge}
                          </div>
                        </div>""", unsafe_allow_html=True)
