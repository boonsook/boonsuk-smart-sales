...
USERS = {
    "admin": "fd9b7e58afbf5850ca3943b6c7c931fb7ff8ef72942a810dc1da014dc0cf01f1",
...
def make_job_line_text(job) -> str:
    """สร้างข้อความ LINE สำหรับงานซ่อม/ติดตั้ง (รูปแบบ card)"""
    name    = safe_text(job.get("customer_name",""))
    phone   = safe_text(job.get("customer_phone",""))
    addr    = safe_text(job.get("customer_address",""))
    fee     = fmt_baht(job.get("net_total", 0))
    dt      = safe_text(job.get("date",""))
    by_who  = safe_text(job.get("saved_by",""))
    status  = safe_text(job.get("status",""))
    model   = safe_text(job.get("model",""))
    section = safe_text(job.get("section",""))
    btu_val = job.get("model_btu", 0)
    try:
        btu_str = f"{int(btu_val):,} BTU"
    except Exception:
        btu_str = str(btu_val)
    task    = f"{section} {model} {btu_str}".strip()
    lines = [
        f"📋 {STORE_NAME}",
        "─────────────────",
        f"👤 ลูกค้า: {name}",
...
# ══════════════════════════════════════════════
    ...
    for idx, job in df_show.iterrows():
        ...
        with st.expander(f"{icon} {job.get('customer_name','?')} | {job.get('model','?')} | {job.get('date','?')} | ฿{fmt_baht(job.get('net_total',0))} | {status}"):
            ...
            with st.expander("📝 ดูข้อความ LINE", expanded=False):
                st.text_area("ข้อความ LINE", value=_jlt, height=180, key=f"lt_{idx}")
    ...
    cancelled_count = len(df_log[df_log["status"] == "❌ ยกเลิก"]) if "status" in df_log.columns else 0
    if cancelled_count > 0:
        col_del, col_exp = st.columns([1, 2])
        if col_del.button(f"🗑️ ลบงานยกเลิกทั้งหมด ({cancelled_count} รายการ)",
                          use_container_width=True, type="secondary"):
            df_log = df_log[df_log["status"] != "❌ ยกเลิก"].reset_index(drop=True)
            save_log(df_log)
            st.success(f"ลบงานที่ยกเลิก {cancelled_count} รายการแล้ว ✅")
            st.rerun()
    else:
        col_exp = st
