...
{{"src":"{_icon512}","sizes":"512x512","type":"image/png","purpose":"any maskable"}}]}}'''
    ...
    <style>
    /* ═══════════════════════════════════════════
       🎨 GLOBAL UI IMPROVEMENTS — BOONSUK v5+
    ═══════════════════════════════════════════ */
    html, body, [class*="css"] {{
    ...
    .stExpander summary {{
        ...
        background: linear-gradient(90deg, #f8faff 0%, white 100%) !important;
    }}
    ...
    .metric-purple {{ border-top-color: #7c3aed; }}
    .metric-teal   {{ border-top-color: #0891b2; }}
    .metric-red    {{ border-top-color: #dc2626; }}
    .metric-blue   h2 {{ color: #1e40af; }}
    .metric-green  h2 {{ color: #15803d; }}
    .metric-orange h2 {{ color: #c2410c; }}
    .metric-purple h2 {{ color: #6d28d9; }}
    .metric-teal   h2 {{ color: #0e7490; }}
    .metric-red    h2 {{ color: #b91c1c; }}
    ...
    .price-summary h4 {{ margin:0 0 4px 0; font-size:12px; opacity:0.8; font-weight:600; }}
    .price-summary h2 {{ margin:0; font-size:28px; font-weight:800; }}
    .price-summary small {{ font-size:11px; opacity:0.75; }}
...
def login_page():
    ...
    .main .block-container {
        ...
        background: linear-gradient(160deg, #1e3a8a 0%, #1d4ed8 50%, #3b82f6 100%) !important;
        min-height: 100vh;
    }
    .login-wrap {
        max-width: 420px; margin: 0 auto; padding: 20px 14px 14px;
    ...
    .login-logo-wrap {
        ...
        box-shadow: 0 8px 24px rgba(30,58,138,0.4); overflow: hidden;
    }
    .login-title { font-size: 18px; font-weight: 800; color: #1e3a8a; margin: 0 0 3px; }
    .login-sub   { font-size: 12px; color: #64748b; margin-bottom: 6px; }
    .login-badge {
        display: inline-block;
    ...
    @media (max-width: 480px) {
        .login-wrap { padding: 12px 8px; }
        .login-card { padding: 24px 14px 18px; }
    }
    </style>
...
/* logout button pink */
[data-testid="stButton"]:has(button[key="hm_logout"]) button,
...
# ══════════════════════════════════════════════
    ...
    m1.markdown(f'<div class="metric-card metric-blue"><h4>📋 งานทั้งหมด</h4><h2>{total_jobs}</h2></div>', unsafe_allow_html=True)
    m2.markdown(f'<div class="metric-card metric-green"><h4>💰 ยอดรวม (฿)</h4><h2>{fmt_baht(total_sales)}</h2></div>', unsafe_allow_html=True)
    m3.markdown(f'<div class="metric-card metric-teal"><h4>✅ รับเงินแล้ว (฿)</h4><h2>{fmt_baht(total_paid)}</h2></div>', unsafe_allow_html=True)
    m4.markdown(f'<div class="metric-card metric-purple"><h4>📊 เฉลี่ย/งาน (฿)</h4><h2>{fmt_baht(avg_sale)}</h2></div>', unsafe_allow_html=True)
    m5.markdown(f'<div class="metric-card metric-orange"><h4>🎉 รับเงินแล้ว</h4><h2>{jobs_paid} งาน</h2></div>', unsafe_allow_html=True)
    m6.markdown(f'<div class="metric-card metric-red"><h4>⏳ รอดำเนินการ</h4><h2>{jobs_wait} งาน</h2></div>', unsafe_allow_html=True)
...
.hg-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:10px;}
.hg-stat{
...
.hg-stat:active{transform:scale(0.95);box-shadow:0 2px 6px rgba(37,99,235,0.2);}
.hg-stat .hg-em{font-size:22px;line-height:1;}
.hg-stat .hg-st{font-size:9px;color:rgba(255,255,255,0.75);letter-spacing:0.3px;}
.hg-stat strong{font-size:14px;color:#fff;font-weight:800;}
...
.hg-card{
  ...
  display:flex;flex-direction:column;align-items:center;
  justify-content:center;gap:5px;min-height:88px;user-select:none;
  transition:transform 0.15s,box-shadow 0.15s,border-color 0.15s;}
.hg-card:hover{box-shadow:0 6px 18px rgba(37,99,235,0.15);border-color:#93c5fd;transform:translateY(-2px);}
.hg-card:active{opacity:0.75;transform:scale(0.96);}
.hg-logout{background:linear-gradient(135deg,#fff1f2,#fff5f5)!important;border-color:#fecaca!important;}
.hg-logout:hover{border-color:#f87171!important;box-shadow:0 6px 18px rgba(220,38,38,0.12)!important;}

.hg-em{font-size:30px;line-height:1;display:block;filter:drop-shadow(0 1px 2px rgba(0,0,0,0.1));}
.hg-lb{font-size:11.5px;font-weight:700;color:#1e3a5f;line-height:1.3;display:block;}
